...
def index():
    ...
    preds_comm = Prediction.query.join(Judgments, Prediction.judgment_id==Judgments.id).order_by(-Prediction.jdgdate).limit(5).all()
    ...
    monthname = dates[-2].strftime("%B") if len(dates) > 2 else ''
...
def list_comm(page=1):
    ...
    time_filter = {
        ...
        'ly': and_(Prediction.jdgdate > begin_time - relativedelta(years=1), Prediction.jdgdate < begin_time),
        'l3m': and_(Prediction.jdgdate > begin_time - relativedelta(months=3), Prediction.jdgdate < begin_time),
