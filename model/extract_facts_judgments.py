...
class JudgmentNoTextError(Exception):
    pass


def extract_parts_judgments(f):  # extract text from different parts

    nn = ['section', 'case_name', 'app_no', 'is_judgement', 'strasbourg' 'date',
          'final', 'intro' 'procedure', 'circumstances', 'domestic_law', 'law',
          'decision', 'end', 'opinions']

    f = [i for i in f if i != '\n']
    print('Extracting facts', len(f))
    if len(f) <= 1:
        raise JudgmentNoTextError
    #print(f[:10], '\n')
    section, case_name, app_no, date, final_date, procedure, circumstances = False, False, False, False, 'N/A\n', False, False

    ###section, grand chamber###
    for num in range(min(10, len(f))):
        section_index = 0
        try:
            line = f[num]
        except IndexError:
    ...
    for num in range(len(f)):
        if DATE and re.search('FINAL', f[num]):
            final_date = f[num+1]
